...
{
    mVulkanDeviceImpl->CreateImage(width, height, format, tiling, usage, properties, image, imageMemory);
}

void VulkanDevice::TransitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout)
{
    mVulkanDeviceImpl->TransitionImageLayout(image, format, oldLayout, newLayout);
}

void VulkanDevice::CopyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height)
{
    mVulkanDeviceImpl->CopyBufferToImage(buffer, image, width, height);
}
...
void VulkanDeviceImpl::CreateImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling,
                                   VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage &image,
                                   VkDeviceMemory &imageMemory)
{
...
void VulkanDeviceImpl::TransitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout,
...
void VulkanDeviceImpl::CopyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height)
{
    VkCommandBuffer commandBuffer = BeginSingleTimeCommands();
